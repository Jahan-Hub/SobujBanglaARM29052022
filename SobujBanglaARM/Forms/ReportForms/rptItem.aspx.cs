...
namespace SobujBanglaARM.Forms
{
    public partial class rptItem : System.Web.UI.Page
    {
        ...
        protected void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    lblMessage.Text = "Select Start Date.";
            //}
            //if (dpEndDate.SelectedDate == null)
            //{
            //    lblMessage.Text = "Select End Date.";
            //}
            //else
            //{
            try
            {
                ...
                Cmd.Parameters.Add("@mode", SqlDbType.VarChar).Value = 222;
                if (cmCommon.SelectedValue != "")
                    Cmd.Parameters.Add("@ItemCode", SqlDbType.VarChar).Value = cmCommon.SelectedValue;
                //if (cmItemSize.SelectedValue != "")
                //    Cmd.Parameters.Add("@ItemSize", SqlDbType.Int).Value = cmItemSize.SelectedValue;
                Cmd.Parameters.Add("@IdClient", SqlDbType.VarChar).Value = AppEnv.Current.p_IdClient;
                ...
                string tempPath = "";
                string reportName = "";
                AppEnv.Current.p_rptObject = "~/Reports/Items.rpt";
                tempPath = @System.IO.Path.GetTempPath() + "ItemsInfo";
                reportName = "ItemsInfo";
        ...
        protected void cmItemSize_ItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
        {
            if (cmCommon.SelectedValue != "")
            {
                try
                {
                    ...
                    Cmd.Parameters.Add("@IdClient", SqlDbType.VarChar).Value = AppEnv.Current.p_IdClient;
                    Cmd.Parameters.Add("@ItemCode", SqlDbType.Int).Value = cmCommon.SelectedValue;
                    SqlDataAdapter adapter = new SqlDataAdapter(Cmd);
                    DataTable dt = new DataTable();
                    adapter.Fill(dt);
                    foreach (DataRow dataRow in dt.Rows)
